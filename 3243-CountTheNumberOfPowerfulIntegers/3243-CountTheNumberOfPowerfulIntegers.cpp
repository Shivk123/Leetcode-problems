// Last updated: 5/28/2025, 9:49:07 PM
class Solution {
public:
    long long numberOfPowerfulInt(long long start, long long finish, int limit,
        ...
        const string aWithLeadingZeros =
            string(b.length() - a.length(), '0') + a;
        vector<vector<vector<long>>> mem(
            b.length(), vector<vector<long>>(2, vector<long>(2, -1)));
        const string sWithLeadingZeros =
            string(b.length() - s.length(), '0') + s;
...
private:
    long count(const string& a, const string& b, int i, int limit,
        ...
        if (i + s.length() == b.length()) {
            const string aMinSuffix =
                tight1 ? std::string(a.end() - s.length(), a.end())
                       : string(s.length(), '0');
            const string bMaxSuffix =
                tight2 ? std::string(b.end() - s.length(), b.end())
                       : string(s.length(), '9');
