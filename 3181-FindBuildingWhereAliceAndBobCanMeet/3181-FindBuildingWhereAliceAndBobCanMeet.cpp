// Last updated: 5/28/2025, 9:49:17 PM
struct IndexedQuery {
    int queryIndex;
    int a; // Alice's index
    int b; // Bob's index
    ...
    vector<int> leftmostBuildingQueries(vector<int>& heights,
        ...
        for (const auto& [queryIndex, a, b] : getIndexedQueries(queries)) {
            if (a == b || heights[a] < heights[b]) {
                ...
                // Binary search to find the smallest index j such that j > b
                // and heights[j] > heights[a], thereby ensuring heights[j] >
                // heights[b].
